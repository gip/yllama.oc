...
async fn model_set_blocks(canister: &Canister<'_>, blocks: &Vec<Principal>) -> () {
    let caller: AsyncCaller<()> = 
        canister.update("model_set_blocks").with_args((blocks,)).build();
    let _: () = caller.call_and_wait().await.expect("call");
}

async fn upload_list(canister: &Canister<'_>) -> Vec<Descr> {
    let caller: AsyncCaller<(Vec<Descr>,)> = 
        canister.update("upload_list").with_args(()).build();
...
async fn upload(canister: &Canister<'_>, name: String, new: bool, data: &[u8]) {
    let caller: AsyncCaller<()> = 
        canister.update("upload").with_args((name, new, data)).build();
...
async fn tensor_list(canister: &Canister<'_>) -> Vec<Descr> {
    let caller: AsyncCaller<(Vec<Descr>,)> = 
        canister.update("tensor_list").with_args(()).build();
    let (r, ) = caller.call_and_wait().await.expect("call");
    r
}
...
async fn model_forward(canister: &Canister<'_>, token: u32, pos: u32) -> u32 {
    let caller: AsyncCaller<(u32,)> = 
        canister.update("model_forward").with_args((token, pos)).build();
...
async fn main() {
    ...
    let json: serde_json::Value =
        serde_json::from_str(&content).expect("canister_ids.json file");
    let j0 = json.as_object().expect("Canister list in JSON");
    let canister_map: HashMap<&String, &str> =
        j0.iter().map(|(k, c)| (k, c.as_object().expect("object").get(target).expect("target").as_str().expect("canister name"))).collect();
   let canister_id: &str =  match canister_map.get(&args.canister) {
    ...
    let (settings,) =
        manager.canister_status(&canister_principal).call_and_wait().await.expect("settings");
    ...
    match args.upload {
        Some(str) => {
            ...
            let b = 2_000_000;
            while c < len {
                upload(&canister, name.to_string(), c == 0, &data[c..std::cmp::min(c+b, len)]).await;
                c += b;
            }
        },
        None => ()
    ...
    match args.set_blocks {
        ...
        Some(s) => {
            ...
            let blocks: Vec<Principal> = split
                .iter()
                .map(|b| 
                    Principal::from_text(canister_map.get(&b.to_string()).expect(&format!("canister {}", b))).expect("principal")
                )
                .collect();
            model_set_blocks(&canister, &blocks).await;
            //let my_principal = Principal::from_slice(identity.public_key().expect("public key").as_slice());
            // println!("AAA {}", my_principal);
            for b in blocks {
